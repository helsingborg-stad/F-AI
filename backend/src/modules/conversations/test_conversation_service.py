...
async def do_test_conversation_service_add_message(service: IConversationService):
    ...
    await service.add_message_to_conversation(conversation_id, timestamp, 'system', 'Answer truthfully')
    await service.add_message_to_conversation(conversation_id, timestamp, 'user', 'What is 2+2')
    await service.add_message_to_conversation(conversation_id, timestamp, 'assistant', '4')
...
async def do_test_conversation_service_set_title(service: IConversationService):
    ...
    await service.set_conversation_title(conversation_id, 'my title')
    conversation = await service.get_conversation(conversation_id)
