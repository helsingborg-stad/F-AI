...
class SettingKey(metaclass=SettingKeyMeta):
    ...
    OTP_EXPIRE_SECONDS = Setting('login.otp_expire_seconds', 900)  # 15 min
    JWT_USER_SECRET = Setting('jwt.user_secret', 'CHANGE THIS')
    JWT_EXPIRE_SECONDS = Setting('jwt.expire_seconds', 900)  # 15 min
    REFRESH_TOKEN_EXPIRE_SECONDS = Setting('refresh_token.expire_seconds', 30 * 24 * 60 * 60)  # 30 days
