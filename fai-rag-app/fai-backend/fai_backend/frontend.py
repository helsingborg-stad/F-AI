...
class ProductionEnvironment(FrontendEnvironment):
    ...
    def configure(self, app: FastAPI):
        app.mount('/assets', StaticFiles(directory=os.path.join(self.static_dir, 'assets')), name='assets')


def get_frontend_environment(env_mode: str, static_dir: str = 'static') -> FrontendEnvironment:
    if env_mode == 'production':
        return ProductionEnvironment(os.path.join(os.getcwd(), 'fai_backend', static_dir))
    else:
