<script lang="ts">
  ...
  import {
    ...
    normalizeFlatten,
    normalizeToArray,
  } from '$lib/components/table/array'
  ...
  const mapOptions = (options: [string, string][]): [string, string, boolean | null][] =>
    options
      .map(([value, label]) => ({
        value,
        label,
  ...
  $: options = mapOptions(
    column?.filterOptions ??
      getDistinct(normalizeFlatten($preFilteredValues)).map((v) => [v, v]),
  )
          ...
          {#key showReset}
            <div class="join">
              <Button
                ...
                size="xs"
                circle
                class={[!showReset ? 'opacity-0' : null, 'join-item']
                  .filter((v) => v && v?.length && v.trim().length > 0)
                  .join(' ')}
                disabled={!showReset}
    ...
    <svelte:fragment>
      {#key showReset}
        <MultiSelect
          ...
          {options}
          on:inputChange={(e) =>
            $filterValue !== e.detail ? ($filterValue = e.detail) : null}
        />
      {/key}
    </svelte:fragment>
  </FormControl>
