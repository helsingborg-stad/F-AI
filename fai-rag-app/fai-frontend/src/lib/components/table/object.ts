...
const getPropertyRecursive = (obj: any, pathParts: string[], defaultValue: any): any => {
    ...
    const [prop, index] = isArrayAccess ? currentPart.split(/\[|\]/).filter(Boolean) : [currentPart];
    ...
    return isArrayAccess && Array.isArray(nextObj)
        ...
        : Array.isArray(obj)
            ? obj.map(item => getPropertyRecursive(item, [prop, ...remainingParts], defaultValue))
            : isObject(obj)
...
const getPropertyFromList = (list: any[], path: string, defaultValue = undefined): any[] =>
    list.map(item => getProperty(item, path, defaultValue));

export const accessProperty = (input: any, path: string, defaultValue: any = undefined): any =>
    Array.isArray(input) ? getPropertyFromList(input, path, defaultValue) : getProperty(input, path, defaultValue);