<script lang="ts">
    ...
    export let html_type: 'text' | 'password' | 'hidden' | 'number' | 'email' | 'tel' | 'search' = 'text';
...
<FormControl {name} {size} {error} {label} class={readonly === true ? "pointer-events-none" : null}>
    <div
    ...
    >
        ...
        <slot name="prefix"/>
        <Input
                class="grow max-h-full max-w-full"
                {name}
                {id}
                {placeholder}
                {readonly}
