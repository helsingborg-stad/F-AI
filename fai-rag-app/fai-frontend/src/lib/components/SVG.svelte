<script lang="ts">
    ...
    export let state: 'primary' | 'secondary' | 'accent' | 'success' | 'info' | 'warning' | 'error' | 'neutral' | null = null
</script>


<svg use:inlineSvg={src} {width} class={`svelte ${className}`} height="auto"
     ...
     class:text-neutral={state === 'neutral'}
     {...$$restProps}
/>
