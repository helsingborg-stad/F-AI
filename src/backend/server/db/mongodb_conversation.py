...
class ConversationRepository(ConversationRepositoryModel):
    ...
    async def find_all(self, q: Dict[str, Any] = {}) -> List[ConversationModel]:
        return await (
            ConversationCollection.find(q)
            .sort("-timestamp.modified", "-timestamp.created")
            .to_list()
        )

    async def insert_message(
        self, conversation_id: str, input_messages: List[InsertMessageModel]
    ) -> ConversationModel:
        messages = [
            MessageModel.parse_raw(message.json()) for message in input_messages
        ]
        return await ConversationCollection.find_one(
            ConversationCollection.id == PydanticObjectId(conversation_id)
        ).update(
            {
                "$push": {ConversationCollection.messages: {"$each": messages}},
                "$set": {ConversationCollection.timestamp.modified: datetime.now()},
            },
...
def init_db_on_start(app: FastAPI):
    ...
    async def init_db():
        ...
        await init_beanie(
            database=client.fai_db_local, document_models=[ConversationCollection]
        )
