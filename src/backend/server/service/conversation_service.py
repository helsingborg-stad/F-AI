...
from ..db.model.conversation_repository import ConversationRepositoryModel
...
from backend.server.schema.response_schema import OutputConversation
from ..db.model.conversation import (
    ConversationModel,
...
def message_exists_in_conversation(
    message_id: int,
) -> Condition[Optional[ConversationModel]]:
    return lambda conversation: conversation and message_id < len(conversation.messages)


def feedback_does_not_exist_for_message(
    message_id: int,
) -> Condition[Optional[ConversationModel]]:
    return (
        lambda conversation: conversation
        and conversation.messages[message_id]
        and len(conversation.messages[message_id].feedback) == 0
    )
...
class ConversationService:
    ...
    async def create_conversation(
        self, input: InputConversation, user: str
    ) -> OutputConversation:
        result = await self.conversations.insert(
            InsertConversationModel(
                messages=input.messages, created_by=user, participants=[user]
            )
        )
    ...
    async def find_all(self, q: Dict[str, Any] = {}) -> List[OutputConversation]:
        ...
        return [
            OutputConversation.parse_raw(conversation.json()) for conversation in result
        ]

    async def add_messages(
        self, conversation_id: str, input_messages: List[InputMessage]
    ) -> OutputConversation:
        result = await self.conversations.insert_message(
            conversation_id, input_messages
        )
        return OutputConversation.parse_raw(result.json())
...
def conversation_service_factory(
    repository_factory: Callable[[], Repository] = create_repository
